        ...
        <table class="table align-middle mb-0">
            <thead style="background-color:#f8f9fa; color:#555; font-size:12px; text-transform:uppercase; border-bottom: 2px solid #b3743b;">
                <tr>
                    ...
                    <th style="width: 120px;">Dokumen TA</th>
                    <th style="width: 110px;">Pembimbing</th>
                    <th style="width: 110px;">Pengulas I</th>
                    <th style="width: 110px;">Pengulas II</th>
                    <th style="width: 150px;">Jadwal Sidang</th>
                    <th style="width: 100px;">Hasil</th>
                </tr>
            </thead>
            <tbody style="font-size:14px;">
                @forelse($sidangAkhirs as $index => $sidang)
                <tr class="hover-row">
                    <td>
                        <input type="checkbox" class="sidang-checkbox" name="selected_sidangs[]" value="{{ $sidang->id }}" {{ $sidang->status == 'approved' ? 'disabled' : '' }}>
                    </td>
                    <td>{{ $index + 1 }}</td>
                    ...
                    <td>
                        ...
                        @if($sidang->file_dokumen_ta)
                            <a href="{{ route('storage.file', ['path' => $sidang->file_dokumen_ta]) }}" target="_blank">Dokumen TA</a>
                        @else
                            <span class="text-muted">-</span>
                        @endif
                    </td>
                    <td>{{ $sidang->pembimbing ?? '-' }}</td>
                    <td>{{ $sidang->pengulas_1 ?? '-' }}</td>
                    <td>{{ $sidang->pengulas_2 ?? '-' }}</td>
                    <td>
                    ...
                    <td>
                        <button
                            ...
                            data-mahasiswa="{{ $sidang->nama }} @if($sidang->nim) ({{ $sidang->nim }}) @endif"
                            data-judul="{{ $sidang->judul }}"
                            data-pembimbing="{{ $sidang->pembimbing }}"
                            data-pengulas1="{{ $sidang->pengulas_1 ?? '' }}"
                            data-pengulas2="{{ $sidang->pengulas_2 ?? '' }}"
                            ...
                            data-file_log_activity="{{ $sidang->file_log_activity ? asset('storage/' . $sidang->file_log_activity) : '' }}"
                            data-file_persetujuan="{{ $sidang->file_persetujuan ? asset('storage/' . $sidang->file_persetujuan) : '' }}"
                            data-revisi_dokumen="{{ $sidang->revisi_dokumen ? asset('storage/' . $sidang->revisi_dokumen) : '' }}"
                            data-form_revisi="{{ $sidang->form_revisi ? asset('storage/' . $sidang->form_revisi) : '' }}"
                        >
                            Details
                        </button>
                    </td>
                ...
                @empty
                <tr>
                    <td colspan="10" class="text-center text-muted py-4">Tidak ada data sidang akhir</td>
                </tr>
                @endforelse
            </tbody>
        </table>
...
$(document).ready(function() {
    ...
    $('#saveSimpan').click(function() {
        ...
        fileInputs.each(function() {
            ...
            if (file) {
                const sid = $(input).data('sidang-id');
                const fd = new FormData();
                fd.append('jadwal_sidang_file', file);
                fd.append('sidang_id', sid);
                fileUploads.push({ formData: fd, sidangId: sid });
            }
        });
        ...
        Promise.all(fileUploads.map(item =>
            fetch('{{ route("koordinator.upload.sidang.jadwal") }}', {
                method: 'POST',
        ...
        .then(uploadResults => {
            ...
            uploadResults.forEach((res, idx) => {
                const sid = fileUploads[idx].sidangId;
                if (res.success) {
                    ...
                    if ($(selectorLink).length) {
                        ...
                        const inputEl = $('.jadwal-file-input[data-sidang-id="' + sid + '"]');
    ...
    $('.btn-selengkapnya').click(function() {
                    ...
                    '<div class="tabs-container">' +
                        ...
                        '<div class="tabs-content">' +
                            ...
                            generateTab('mahasiswa', ['Dokumen TA', 'Log Activity', 'Form Persetujuan', 'Revisi Dokumen', 'Form Revisi'], [
                                { dataKey: 'fileDokumenTa', field: 'file_dokumen_ta' },
        ...
        modal.on('change', '.file-input', function() {
            ...
            fetch('{{ route("koordinator.upload.sidang.dokumen") }}', {
                method: 'POST',
                headers: { 'X-CSRF-TOKEN': '{{ csrf_token() }}' },
                body: fd
            })
            ...
            .then(res => {
                if (res.success) {
                    ...
                    if (hiddenPath.length === 0) {
                        item.find('.file-actions').append('<div class="hidden-file-path" style="display:none;" data-path="' + newPath + '"></div>');
                    } else {
